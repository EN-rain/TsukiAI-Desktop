...
- Deployed service source: pending the JP-routing commit (Free.ai-matched target, API-side denoise/loudness mastering, and bridge peak ceiling); the VM uses `OPENVOICE_BASE_SPEAKER_EN=JP`, `OPENVOICE_SPEED_EN=1.0`, `OPENVOICE_SPEED_JA=1.0`, and `OPENVOICE_OUTPUT_SAMPLE_RATE=24000`.
...
- Prior live smoke verification: the exact HAR text returned a valid mono PCM16 24 kHz WAV; one earlier live run was 11.668042 seconds, and direct converter embedding cosine against the HAR WAV was 0.90618801.
...
- HAR-match recheck (historical): the active original-reference target measured cosine `0.7564` against the exact Free.ai WAV; the preserved Free.ai-derived target measured `0.9802`. The earlier `EN-US` speed-0.8 render was `15.151` seconds versus that older Free.ai output at `15.120` seconds.
- Manual Chrome verification: Free.ai showed `model=openvoice`, speed `1`, format `wav`, and the exact 189-character text; the current output was `12.400` seconds while the matching local `EN-US` speed-1 render was `12.411` seconds. The old HAR output was `15.120` seconds, proving Free.ai's backend output varies between runs even with the same visible settings.
- Live deployment verification: after the speed-1 redeploy, both `openvoice-api.service` and `tsuki-discord-voice-bridge.service` were active. The exact sentence returned a mono PCM16 24 kHz WAV at `12.350` seconds, `-16.456 dB` RMS, and `-1.000 dB` peak. The verified sample was posted to Discord text channel `1545789527556104393` as message `1548218805019353168` with one audio attachment.
- JP-for-English deployment verification: the VM is intentionally active with `OPENVOICE_BASE_SPEAKER_EN=JP`; no EN-US fallback is configured. The exact English benchmark returned `1.650` seconds at `-86.075 dB` RMS and `-66.787 dB` peak.
